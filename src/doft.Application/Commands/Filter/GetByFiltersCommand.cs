...
{
    public class GetByFiltersCommand : IRequest<ApiResponse<List<GetAllItemForUserResponseDto>>>
    {
        ...
        public GetByFiltersCommand(int? year, int? month, int? day, int? categoryId, string? itemType, string userId)
        {
            Year = year;
            Month = month;
            Day = day;
    ...
    public class GetByFiltersCommandHandler : IRequestHandler<GetByFiltersCommand, ApiResponse<List<GetAllItemForUserResponseDto>>>
    {
        private readonly ITaskRepository _taskRepository;
        private readonly IEventRepository _eventRepository;
        private readonly ILogger<GetByFiltersCommandHandler> _logger;

        public GetByFiltersCommandHandler(ITaskRepository taskRepository, IEventRepository eventRepository, ILogger<GetByFiltersCommandHandler> logger)
        {
        ...
        public async Task<ApiResponse<List<GetAllItemForUserResponseDto>>> Handle(GetByFiltersCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                var tasks = await _taskRepository.GetTasksByFilters(request.Year, request.Month, request.Day, request.CategoryId, request.UserId);
                if (tasks != null)
                {
            ...
            {
                var events = await _eventRepository.GetEventsByFilters(request.Year, request.Month, request.Day, request.CategoryId, request.UserId);
                if (events != null)
                {
            ...
            if (response.Count == 0)
            {
                _logger.LogWarning("No items found for the specified filters");
                return ApiResponse<List<GetAllItemForUserResponseDto>>.NotFound("No items found for the specified filters");
            }
